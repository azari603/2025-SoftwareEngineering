...
| **Trigger** | <사용자가 아이디와 비밀번호 입력 후 “로그인” 버튼을 클릭한다.> |
...
| **Success Post Condition** | <입력한 이메일로 가입한 아이디를 표시한다.> |
...
| **1** | <사용자가 설정 항목(비밀번호)을 변경한 후 저장 버튼을 클릭한다.> |
| **2** | <시스템이 변경값을 검증한다.> |
...
| Step | Branching Action |
|---|---|
| **2** |  **2a.  비밀번호 형식 불일치**<br>…**2a1.**  “비밀번호 형식이 올바르지 않습니다.”라는 오류 메시지를 출력한다.|
...
| 항목 | 내용 |
|---|---|
| **Summary** | <사용자가 책BTI 테스트를 수행하고, 유형별 점수를 기반으로 성향 유형을 도출하며, AI 추천 엔진을 통해 해당 유형에 맞는 도서를 추천받는다.> |
| **Scope** | <CHAECK system> |
| **Level** | <User Level> |
| **Author** | <송서현> |
| **Last Update** | <2025.10.27> |
...
| **Success Post Condition** | <시스템이 각 문항의 유형별 점수를 합산하고, 최고 점수에 해당하는 유형을 결정한다.<br>사용자의 책BTI 결과 유형이 화면에 표시된다.<br>AI 추천 엔진이 해당 유형을 기반으로 추천 도서 목록을 생성하여 함께 표시한다.
> |
...
| **3** | <사용자가 각 문항에 응답한다.> |
| **4** | <시스템이 각 응답을 실시간으로 해당 유형의 점수에 누적한다.> |
| **5** | <모든 문항 응답이 완료되면, 시스템이 유형별 총점을 계산한다.> |
...
| 항목 | 내용 |
|---|---|
| **Summary** | <“나의 서재” 페이지에서 사용자가 상태를 저장한 책을 모아볼 수 있다.> |
| **Scope** | <CHAECK system> |
| **Level** | <User Level> |
| **Author** | <송서현> |
| **Last Update** | <2025.10.27> |
...
| **3** | <사용자가 좋아요 버튼을 한번 더 클릭하면 좋아요 취소가 가능하다.> |
...
| **3** | **3a. 좋아요 취소에 실패 했을 경우**<br>…**3a1.** 좋아요 취소에 실패했다는 메시지를 출력한다.|
...
| Step | Action |
|---|---|
| **S** | <사용자는 둘러보기 화면에서 최신 서평 혹은 팔로잉하고 있는 사람들의 서평을 확인할 수 있다.> |
...
| **Primary Actor** | <로그인한 User> |
| **Preconditions** | <사용자가 로그인 상태여야 한다.> |
| **Trigger** | <사용자가 계정 설정 페이지에서 프로필 내용을 수정한 후 저장버튼을 누른다.<br>DB가 사용자 프로필을 저장하고 있다.> |
...
| Step | Action |
|---|---|
| **S** | <사용자가 계정 설정 페이지에서 프로필을 수정한다.> |
| **1** | <시스템이 계쩡 설정 페이지에 기존정보를 출력한다.> |
| **2** | <사용자가 계정 설정 페이지에서 프로필의 각 항목을 변경하고 저장버튼을 누른다.> |
...
| **Trigger** | <사용자가 ‘독서 통계’에서 목표 설정 수정 버튼을 누른 후 팝업에 목표값을 입력하고 완료 버튼을 누른다.> |
...
|---|---|
| **S** | <사용자가 이달의 목표를 설정한다.> |
| **1** | <사용자가 ‘독서 통계’ 페이지에서 나의 목표 설정 버튼을 눌러 목표설정 팝업이 나타난다.> |
...
# Use Case #<26> : <통계 기록 보기>

## GENERAL CHARACTERISTICS

| 항목 | 내용 |
|---|---|
| **Summary** | <사용자가 ‘나의 기록’페이지에서 ‘독서 통계’를 선택해서 독서통계(완독도서, 작성한 서평 수, 이번달 독서량, 이번달 목표 독서량, 이번달 목표 달성률, 도서별 평점, 월별 독서량, 자주 읽은 저자)를 확인 할 수 있다.> |
| **Scope** | <CHAECK system> |
| **Level** | <User Level> |
| **Author** | <정도희> |
| **Last Update** | <2025.10.27> |
...
| **1** | <사용자가 ‘독서 통계’ > ‘자주 읽은 저자의 순위’에서 특정 저자의 펼치기 버튼을 클릭한다.> |
